...
@router.get("/active", response_model=List[Medicine])
def get_active_medicines(session: Session = Depends(get_session)):
    return session.exec(select(Medicine).where(Medicine.is_active == True)).all()
